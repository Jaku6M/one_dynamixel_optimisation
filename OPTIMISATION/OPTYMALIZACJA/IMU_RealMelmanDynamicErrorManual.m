...
RealFile = '/home/jaku6m/Desktop/Plots/IMURealMelman/kness_bending_fast.csv';
RealValues = readmatrix(RealFile);
%182 measurements = 1 second. Every 0.00549s data is sampled 
for j = 1:length(RealValues)
    RealValues(j,7) = (j-1)*5.4945;
end
...
%Trim the data that is near to zero
    % WORKS FOR KNEESBENDING:
    % RealValues=trimdata(RealValues,length(RealValues),Side="leading");
    % WORKS FOR WAVINGLEFTHAND
    % RealValues=trimdata(RealValues,length(RealValues)-100,Side="leading");
    % WORKS FOR KNEESBENDINGFAST:
    RealValues=trimdata(RealValues,length(RealValues),Side="leading");
...
Real_time_ms = RealValues(:, 7);       % Time in milliseconds
% Find the indices of time values closest to multiples of 50
multiples_of_50 = 0:50:max(Real_time_ms);
[~, time_indices] = arrayfun(@(x) min(abs(Real_time_ms - x)), multiples_of_50);
% Keep the indices where time values are closest to multiples of 50
RealValues = RealValues(ismember(1:length(Real_time_ms), time_indices), :);
for j = 1:length(RealValues)
    RealValues(j,7) = (j-1)*50;
end

% Update Real_time_ms after extraction Extract columns into separate variables
Real_time_ms = RealValues(:, 7);  % Time in milliseconds
Real_X_G = RealValues(:, 1);  % acceleration in G values multiplied to achieve m/s^2
Real_Y_G = RealValues(:, 2);  % acceleration in G values multiplied to achieve m/s^2
Real_Z_G = RealValues(:, 3);  % acceleration in G values multiplied to achieve m/s^2
Real_X_A = RealValues(:, 4); %velocity in degrees/s
Real_Y_A = RealValues(:, 5); %velocity in degrees/s
Real_Z_A = RealValues(:, 6); %velocity in degrees/s
...
Gazebo_X_G = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_linear_acceleration_x.csv";
Gazebo_Y_G = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_linear_acceleration_y.csv";
Gazebo_Z_G = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_linear_acceleration_z.csv";
Gazebo_X_A = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_angular_velocity_x.csv";
Gazebo_Y_A = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_angular_velocity_y.csv";
Gazebo_Z_A = "/home/jaku6m/Desktop/Plots/IMUGazeboMelman/KneesBendingFast/Plot Name-_gazebo_default_Melman_Right_Thigh_MPU6050sensor_imu:p=_angular_velocity_z.csv";
...
%Trim the data that is equal to zero
    ...
    % WORKS FOR KneesBending
    % GazeboValues=trimdata(GazeboValues,length(GazeboValues)-210,Side="leading");
    % WORKS FOR WavingLeftHand
    % GazeboValues=trimdata(GazeboValues,length(GazeboValues)-186,Side="leading");
    % WORKS FOR KneesBendingFast
    GazeboValues=trimdata(GazeboValues,length(GazeboValues)-228,Side="leading");
    %Adjust the starting time to zero
...
XG_error_abs=sqrt(XG_errorsquared);
YG_error_abs=sqrt(YG_errorsquared);
ZG_error_abs=sqrt(ZG_errorsquared);
XA_error_abs=sqrt(XA_errorsquared);
YA_error_abs=sqrt(YA_errorsquared);
ZA_error_abs=sqrt(ZA_errorsquared);
XG_error_abs_val=sum(XG_error_abs)/length(RealValues);
YG_error_abs_val=sum(YG_error_abs)/length(RealValues);
ZG_error_abs_val=sum(ZG_error_abs)/length(RealValues);
XA_error_abs_val=sum(XA_error_abs)/length(RealValues);
YA_error_abs_val=sum(YA_error_abs)/length(RealValues);
ZA_error_abs_val=sum(ZA_error_abs)/length(RealValues);
...
disp(['Blad sredni dla przyspieszenia X: ', num2str(XG_error_abs_val)]);
disp(['Blad srednidla przyspieszenia Y: ', num2str(YG_error_abs_val)]);
disp(['Blad srednidla przyspieszenia Z: ', num2str(ZG_error_abs_val)]);
disp(['Blad srednidla prędkości X: ', num2str(XA_error_abs_val)]);
disp(['Blad srednidla prędkości Y: ', num2str(YA_error_abs_val)]);
disp(['Blad sredni dla prędkości Z: ', num2str(ZA_error_abs_val)]);
            ...
            errorText = sprintf(['Błędy dynamiczne:\n\n', ...
                                 ...
                                 XG_error_abs_val, YG_error_abs_val, ZG_error_abs_val);
            annotation('textbox',[0.905 .5 .1 .2],'String',errorText,'EdgeColor','none')
            % Real values with 'A'
            figure('Position', [100, 100, 1000, 800]);
            plot(Real_time_ms, Real_X_A, 'DisplayName', 'Real Velocity X axis')
            hold on
            plot(Real_time_ms, Real_Y_A, 'DisplayName', 'Real Velocity Y axis')
            hold on
            plot(Real_time_ms, Real_Z_A, 'DisplayName', 'Real Velocity Z axis')
            hold on
            ...
            plot(Gazebo_time_ms, Gazebo_YAval, 'DisplayName', 'Gazebo Y Velocity')
            hold on
            plot(Gazebo_time_ms, Gazebo_ZAval, 'DisplayName', 'Gazebo Z Velocity', 'LineWidth', 2, 'LineStyle', '--')
            hold on
            xlabel("Czas [ms]")
            ...
            errorText = sprintf(['Błędy dynamiczne:\n\n', ...
                                 ...
                                 XA_error_abs_val, YA_error_abs_val, ZA_error_abs_val);
            annotation('textbox',[0.905 .5 .1 .2],'String',errorText,'EdgeColor','none')
