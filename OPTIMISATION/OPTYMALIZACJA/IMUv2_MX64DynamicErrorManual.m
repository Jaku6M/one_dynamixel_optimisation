...
RealFile = '/home/jaku6m/Desktop/Plots/RealIMUv2_MX64Plots/trajectory7_bis.csv';
...
%Trim the data that is near to zero
    ...
    RealValues=trimdata(RealValues,length(RealValues)-550,Side="leading");
    % WORKS FOR TRAJECTORY 10
    % RealValues=trimdata(RealValues,length(RealValues)-130,Side="leading");
    % WORKS FOR TRAJECTORY 11
    % RealValues=trimdata(RealValues,length(RealValues)-150,Side="leading");
    % Real_Y_A = RealValues(:, 5);
    % first_nonzero=find(Real_Y_A > 1.0 | Real_Y_A < -1.0,1)-40;
...
%Trim the data that is equal to zero
    ...
    % WORKS FOR TRAJECTORY 11
    Gazebo_first_nonzero=find(Gazebo_YAval > 0.01 | Gazebo_YAval < -0.01,1)-10;
